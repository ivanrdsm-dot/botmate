...
function Ring({ label, value, unit }: { label: string; value: number; unit: string }) {
  return (
    <div className="rounded-2xl border p-4 text-center" style={{ borderColor: "rgba(74,222,128,.18)", background: C.bgSoft }}>
      <div className="text-xl font-bold" style={{ color: C.brandLight }}>{value}<span className="ml-1 text-xs font-normal opacity-60">{unit}</span></div>
...
export default function Diario() {
  ...
  function startDraftIfNeeded() { if (!draft) { setDraft([]); setDraftSlot(slotForNow()); } }
  ...
  function addManual() {
    ...
    setDraft([...(draft ?? []), { name: manual.name.trim(), kcal, protein: Number(manual.protein) || 0, carbs: Number(manual.carbs) || 0, fat: Number(manual.fat) || 0, confidence: "alta" }]);
  ...
  return (
    <div className="space-y-8">
      <div><h1 className="text-3xl font-bold">Diario de hoy 📸</h1><p className="mt-1 text-sm opacity-70">Foto de tu plato → la IA estima calorías y macros.</p></div>
      ...
      {target > 0 && (
        <div>
          ...
          <div className="mt-1 text-xs opacity-60">{Math.round(totals.kcal)} / {target} kcal {over && <span style={{ color: C.accent }}>· pasaste tu meta</span>}</div>
        </div>
      )}
      {!target && <p className="text-xs opacity-60">Crea tu plan en <Link href="/onboarding" className="underline">onboarding</Link> para ver tu meta.</p>}
      <section className="rounded-2xl border p-5 text-center" style={{ borderColor: "rgba(74,222,128,.2)", background: C.bgSoft }}>
        ...
        <div className="flex flex-wrap justify-center gap-2">
          <button onClick={() => inputRef.current?.click()} disabled={analyzing} className="rounded-full px-6 py-3 font-semibold text-black disabled:opacity-50" style={{ background: C.brand }}>{analyzing ? "Analizando…" : "📷 Tomar / subir foto"}</button>
          <button onClick={startDraftIfNeeded} className="rounded-full border px-5 py-3 text-sm font-semibold" style={{ borderColor: "rgba(74,222,128,.3)", color: C.brandLight }}>➕ Agregar manual</button>
        </div>
      ...
      {draft && (
        <section className="rounded-2xl border p-5" style={{ borderColor: "rgba(74,222,128,.25)", background: C.bgSoft }}>
          <div className="mb-3 flex flex-wrap gap-2">
            {SLOTS.map((s) => (
              <button key={s.id} onClick={() => setDraftSlot(s.id)} className="rounded-full border px-3 py-1.5 text-sm"
                style={{ borderColor: draftSlot === s.id ? C.brand : "rgba(74,222,128,.25)", background: draftSlot === s.id ? C.brand : "transparent", color: draftSlot === s.id ? "#000" : "#E6F4EC" }}>
                {s.emoji} {s.label}
              </button>
            ))}
          </div>
            ...
            {draft.map((it, i) => (
              <div key={i} className="flex items-start justify-between gap-3 text-sm">
                <div className="min-w-0">
                  <div className="font-medium">{it.name}{it.confidence && it.confidence !== "alta" && <span className="ml-2 rounded-full px-2 py-0.5 text-[10px]" style={{ background: "rgba(245,158,11,.15)", color: C.accent }}>confianza {it.confidence}</span>}</div>
          ...
          <div className="mt-4 rounded-xl border p-3" style={{ borderColor: "rgba(74,222,128,.15)" }}>
            ...
            <button onClick={addManual} className="mt-2 rounded-full border px-4 py-1.5 text-xs" style={{ borderColor: "rgba(74,222,128,.3)", color: C.brandLight }}>+ Añadir alimento</button>
          </div>
          {notes && <p className="mt-3 rounded-xl border p-3 text-xs" style={{ borderColor: "rgba(245,158,11,.3)", color: "#FCD9A0" }}>{notes}</p>}
      ...
      <section>
        ...
        {day.length === 0 ? <p className="mt-2 text-sm opacity-60">Aún no registras nada hoy. Toma tu primera foto. 👆</p> : (
          <div className="mt-4 space-y-5">
            {SLOTS.map((s) => {
              ...
              return (
                <div key={s.id}>
                  <div className="mb-2 flex items-center justify-between text-sm"><span className="font-semibold">{s.emoji} {s.label}</span><span className="opacity-60">{Math.round(sub)} kcal</span></div>
                  <div className="space-y-2">
                    {entries.map((e) => (
                      <div key={e.id} className="rounded-2xl border p-4" style={{ borderColor: "rgba(74,222,128,.15)", background: C.bgSoft }}>
                        <div className="flex items-center justify-between"><span className="text-sm opacity-80">{e.items.map((i) => i.name).join(", ")}</span><span className="text-sm font-semibold" style={{ color: C.brandLight }}>{Math.round(e.kcal)} kcal</span></div>
