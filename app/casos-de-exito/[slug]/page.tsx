...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}){const {slug}=await params;const own=botmateCases.find(r=>r.slug===slug);if(own)return localizedMetadata(own.brand+" · Proyecto Botmate en México",own.description.es,"/casos-de-exito/"+slug);const r=stories.cases.find(r=>r.slug===slug);return r?localizedMetadata(r.brand+" · Caso internacional Pudu",r.description.es,'/casos-de-exito/'+r.slug):{};}
