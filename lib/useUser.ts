...
export function useUser() {
  ...
  useEffect(() => {
    ...
    const map = (u: { id: string; email?: string; user_metadata?: Record<string, unknown> } | null): VitalaUser | null =>
      u ? {
    ...
    sb.auth.getUser().then(({ data }) => { setUser(map(data.user)); setLoading(false); });
    const { data: sub } = sb.auth.onAuthStateChange((_event, session) => { setUser(map(session?.user ?? null)); });
