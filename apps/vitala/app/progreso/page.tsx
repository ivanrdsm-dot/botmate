            ...
            {delta !== null && (
              ...
              >
                <div
                  ...
                  style={{ color: delta < 0 ? C.brandLight : C.accent }}
                >
                  {delta > 0 ? "+" : ""}
                  {delta.toFixed(1)} kg
                </div>
          ...
          <button
            ...
            style={
              tab === t
                ...
                : { border: `1px solid rgba(74,222,128,.25)`, color: C.brandLight }
            }
          >
            {t === "peso" ? "Peso" : "Medidas"}
          </button>
        ...
        <div className="space-y-6">
          ...
          <Reveal>
            <form
              ...
              style={{ borderColor: "rgba(74,222,128,.18)", background: C.bgSoft }}
            >
              <h2 className="font-semibold" style={{ color: C.brandLight }}>
                Registrar peso
              </h2>
              <div className="grid gap-3 sm:grid-cols-3">
                ...
                <div>
                  <label className="mb-1 block text-xs opacity-70">Peso (kg)</label>
                  <input
                    type="number"
                    step="0.1"
                    min="20"
                ...
                <div>
                  <label className="mb-1 block text-xs opacity-70">Nota (opcional)</label>
                  <input
                    type="text"
              ...
              <div className="space-y-2">
                ...
                {[...entries].reverse().slice(0, 20).map((e) => (
                  <div
                    ...
                    style={{ borderColor: "rgba(74,222,128,.12)", background: C.bgSoft }}
                  >
                    <div>
                      <span className="font-semibold" style={{ color: C.brandLight }}>
                        {e.kg} kg
                      </span>
                      <span className="ml-3 opacity-60 text-xs">{e.date}</span>
                      {e.notes && (
                        <span className="ml-3 opacity-50 text-xs">{e.notes}</span>
                      )}
                    </div>
                    <button
                      onClick={() => handleDelete(e.id)}
                      className="rounded-lg px-2 py-1 text-xs opacity-40 hover:opacity-80 transition-opacity"
                    >
                      ✕
                    </button>
                  </div>
            ...
            >
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {[
                  ...
                  <div key={key}>
                    <label className="mb-1 block text-xs opacity-70">{lbl}</label>
                    <input
                      type="number"
                      step="0.1"
                      min="0"
                      placeholder="—"
                      value={mForm[key as keyof typeof mForm]}
                      onChange={(e) => setMForm((prev) => ({ ...prev, [key]: e.target.value }))}
                      className={inp}
                      style={inpStyle}
                    />
                  </div>
      ...
      {entries.length === 0 && (
        <Reveal>
          <div
            ...
            style={{ borderColor: "rgba(74,222,128,.15)" }}
          >
            <p>Aún no tienes registros. Agrega tu peso de hoy para empezar.</p>
            <p className="mt-1 text-xs opacity-60">
              Se recomienda pesarse a la misma hora, idealmente en ayunas.
            </p>
          </div>
        </Reveal>
      )}
