...
export default function PuduHero({locale='es'}:{locale?:Locale}) {
 ...
 const advance=useCallback(()=>setIndex(i=>(i+1)%featured.length),[]);
 ...
 return <section ref={root} onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)} onFocusCapture={e=>{if(!(e.target as HTMLElement).closest("[data-rotation-control]"))setPaused(true);}} className="pudu-hero" aria-label={es?'Robótica Pudu con Botmate':'Pudu robotics with Botmate'}>
  <div className="container-x pudu-hero-grid">
   <div className="pudu-hero-copy">
    ...
    <div className="hero-actions"><Link className="btn-primary" href={localPath('/encuentra-tu-robot',locale)}>{es?'Encuentra tu robot':'Find your robot'}<ArrowUpRight size={18}/></Link><Link className="text-link" href={localPath('/reservar',locale)}>{es?'Hablemos de tu proyecto':'Let’s talk about your project'}<ArrowUpRight size={17}/></Link></div>
