...
export default function ExperienceCinema({locale = 'es'}: {locale?: Locale}) {
  ...
  return <section id="en-accion" className="experience-cinema">
    <div className="container-x">
      <div className="cinema-heading"><h2>{es ? 'Robots en acción' : 'Robots in action'}</h2><p>{es ? 'Mira una demostración y explora sus funciones, aplicaciones y refacciones.' : 'Watch a demonstration, then explore functions, applications and parts.'}</p></div>
      ...
      <div className="cinema-caption"><div aria-live="polite"><span>{es ? 'VIDEO OFICIAL / PUDU ROBOTICS · SIN AUDIO' : 'OFFICIAL FILM / PUDU ROBOTICS · NO AUDIO'}</span><h3>{titles[index]}</h3></div><Link className="text-link" href={localPath('/robots/' + robot.slug, locale)}>{es ? 'Explorar funciones' : 'Explore functions'}<ArrowUpRight size={20}/></Link></div>
    </div>
  </section>;
}
